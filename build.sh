...
docker build --no-cache -t cleanup-cli:$VERSION --build-arg VERSION=$VERSION .

docker tag cleanup-cli:$VERSION cleanup-cli:latest

echo cleanup-cli:$VERSION > ./docker_images/generated_dockers.txt


# path build.sh with "DUMP_IMAGE=1" to save docker
FILE_NAME=cleanup-cli:${VERSION}.tar
DUMP_OUTPUT_PATH=${OUTPUT_DIR}/releases/${FILE_NAME}

if [[ ! -z "${DUMP_IMAGE}" ]]
then
     docker save -o ./${DUMP_OUTPUT_PATH} cleanup-cli:$VERSION
